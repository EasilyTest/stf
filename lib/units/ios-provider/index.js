...
module.exports = function(options) {
  ...
  log.info('allowAppium: ', options.allowAppium)
  var workers = {}
  ...
  var ports = null
  var devicePorts = {}
  if(!options.allowAppium) {
    ports = options.ports.slice(
      0
    ...
    ports = options.ports.slice(
      0
    , options.ports.length - options.ports.length % 8
    )
  }
    ...
    idevice.on('add', filterDevice(function(device) {
      ...
      function work() {
        return (worker = workers[device.id] = spawn())
          ...
          .finally(function(){
            log.info('remove the device %s frome devicelist',device.id)
  ...
  var ideviceTimer = setInterval(checkDevice, 30000);
